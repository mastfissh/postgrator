...
module.exports = function (config) {
  ...
  var commonClient = {
    ...
    queries: {
      getRunVersions: 'SELECT version FROM ' + config.schemaTable + ' ORDER BY version DESC',
  ...
  if (config.driver === 'mysql') {
    ...
    if (config.schemaTable.indexOf(".") !== -1){
    var table
    var schema
    var strs = config.schemaTable.split(".")
      commonClient.queries.checkTable = "SELECT * FROM pg_catalog.pg_tables WHERE schemaname = '" + strs[0] + "' AND tablename = '" + strs[1] + "';"
      console.log(commonClient.queries.checkTable)
    } else {
